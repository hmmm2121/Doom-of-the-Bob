...
public class PlayerMovement : MonoBehaviour
{
    ...
    public bool autoBounce = true;          // bounce automatically on slam impact, no extra button
    [Range(0f, 1f)]
    ...
    // Ramp downward speed toward slam velocity instead of snapping instantly -> smooth slam
    void SlamDescent()
    {
    ...
    void TriggerSlamImpact()
    {
        ...
        // automatic bounce on impact, scaled by how long we fell (no button needed)
        if (autoBounce)
        {
            float bounceForce = Mathf.Min(
                slamBounceBaseForce + slamBounceForcePerSecond * slamAirTime,
