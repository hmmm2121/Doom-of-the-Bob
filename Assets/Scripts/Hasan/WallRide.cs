namespace Official
{
    ...
    public class WallRide : MonoBehaviour
    {
        ...
        float currentTilt;
        Vector3 camBaseEuler;

        void Start()
        {
            rb = GetComponent<Rigidbody>();
            if (orientation == null) orientation = transform;
            if (cameraTransform != null) camBaseEuler = cameraTransform.localEulerAngles;
        }
        ...
        {
            float targetTilt = WallRunTilt * maxTiltAngle;
            currentTilt = Mathf.Lerp(currentTilt, targetTilt, tiltSpeed * Time.deltaTime);

            if (cameraTransform == null) return;

            Vector3 e = cameraTransform.localEulerAngles;
            cameraTransform.localEulerAngles = new Vector3(e.x, e.y, camBaseEuler.z + currentTilt);
        }
